...
void adc_continue_mode_get(char * hex_str)
{
    uint32_t i;
    gpio_t *gpiox;
    uint32_t pin;
    ...
    {//calibration sample value
		...
		value += calibrated_sample_1[i];
    }
	value /= ADC_DATA_NUM;
	value *= 1000000;
	intvalue = ceil(value);
	printf("%d\r\n",intvalue);
	sprintf(hex_str,"%06x",intvalue);
	printf("%06x\r\n",hex_str);
}
...
static void PrepareTxFrame( uint8_t port )
{
	...
	*/
	...
	printf("%08x\r\n",adc_hex_str);
	StringToHex(adc_hex_str,(char*)AppData,(int*)AppDataSize); // arguments invaild
