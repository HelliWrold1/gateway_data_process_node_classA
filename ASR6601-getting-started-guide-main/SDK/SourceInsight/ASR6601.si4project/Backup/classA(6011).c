...
/*!
 * \brief   Prepares the payload of the frame
 * 将PrepareTxFrame函数准备的待发送数据发出，设置是否需要确认以及刷新Mac命令，返回1:可以发送；0不可以发送
...
int app_start( void )
{
    LoRaMacPrimitives_t LoRaMacPrimitives; //LoRaMAC事件结构 结构体,用于通知上层MAC事件
    LoRaMacCallback_t LoRaMacCallbacks; // LoRaMAC回调结构 结构体
    MibRequestConfirm_t mibReq; // LoRaMAC MIB-Request确认结构 结构体
