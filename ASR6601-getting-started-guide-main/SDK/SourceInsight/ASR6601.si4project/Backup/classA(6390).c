...
static void PrepareTxFrame( uint8_t port )
{
    ...
    AppData[3] = 0x03;
	*/
	printf("prepare tx frame\r\n");
	double result;
	int r;
	// ADC scan
//	adc_continue_mode_get(adc_hex_str);
//	StringToHex(adc_hex_str, AppData, &AppDataSize);
	// i2c transmit
	result = request_data_BH1750(I2C0,0x23);
	printf("%lf\r\n",result);
	result *= 1000000;
	r = ceil(result);
	printf("%d\r\n",r);
	memset((char*)i2c_hex_str,0,20);
	sprintf((char*)i2c_hex_str,"%08x",r);
	StringToHex(i2c_hex_str, AppData, &AppDataSize);
//	unsigned char i;
