...
bool TxDutyCycleTime_instruction_analysis(uint32_t* TxDutyCycleTime, uint8_t cmd, uint8_t h, uint8_t m, uint8_t s, uint8_t* AppData, uint8_t* AppDataSize,bool* TxDutyCycleTimeQuery)
{
	if(cmd == 0x1E)
	{
		...
		{// If query TxDutyCycleTime
			...
			AppData[15] = ( ( (*TxDutyCycleTime / 1000) - AppData[16] ) % 3600 ) / 60;
			AppData[14] = ( (*TxDutyCycleTime / 1000) - AppData[16] - AppData[15] * 60 ) / 3600;
//			printf("time:%x %x %x\r\n",AppData[14],AppData[15],AppData[16]); // Debug Code
			*AppDataSize = 4;
